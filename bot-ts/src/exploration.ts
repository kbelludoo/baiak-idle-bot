...
export function chooseExplorationTarget(
  candidates: Array<{ id: string; can_tank?: boolean; exp_h?: number; gold_h?: number }>,
  ...
  const best = [...viable].sort((a, b) =>
    Number(b.exp_h || 0) - Number(a.exp_h || 0) || Number(b.gold_h || 0) - Number(a.gold_h || 0),
  )[0];
  if (!currentId) return { id: best.id, reason: 'sem hunt atual: iniciar candidato sobrevivível' };
  if (best.id === currentId) return null;
  return { id: best.id, reason: `candidato com melhor XP/h e sobrevivência: ${best.id}` };
}
