async ({ autoHeal = true, healBelowPct = 75, hpPotionBelowPct = 60, manaPotionBelowPct = 65 }) => {
  ...
  const configureOpenedPotionPicker = async (isHp) => {
    ...
    const potName = (bestRow?.querySelector("b, strong, .im-name")?.textContent || bestRow?.textContent || "Potion").split("\n")[0].trim();
  ...
  const configureOpenedHealPicker = async () => {
    ...
    const autoBtn = btns.find((b) => {
      const row = (b.closest(".sp-book-row, .im-row, div")?.textContent || "").toLowerCase();
    ...
    if (autoBtn) {
      ...
      if (btnText === "em uso") {
        ...
        autoBtn.click();
        events.push("EQUIPOU_MAGIA_CURA: " + (autoBtn.closest(".sp-book-row, div")?.textContent || "").slice(0, 30));
        await sleep(250);
      }
    }
    closePicker();
